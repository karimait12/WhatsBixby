...
class Message extends Base {
  ...
  async sendMessage(
    jid,
    content,
    opt = { packname: config.PACKNAME, author: config.AUTHOR , fileName: "Codex" },
    type = "text"
  ) {
  ...
  try {
      ...
      await git.fetch();
      const commits = await git.master(['..origin/master']);
      if (commits.total === 0) {
  ...
  async UpdateHeroku(message) {
    ...
    const commits = await git.log(['..origin/master..HEAD']);
