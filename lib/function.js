...
async function WriteSession() {
  try {
    ...
    if (process.env.SESSION !== undefined) {
      ...
      const encodedId = encodeURIComponent(EncryptedText);
      const { data } = await axios.get(`${BASE_URL}session?id=${encodedId}`);
      const result = data.result[0].data;

      try {
        fs.writeFileSync("./lib/temp/session/creds.json", JSON.stringify(result, null, 2));
        console.log("Session extraction complete");
      } catch (fileError) {
        console.error("Failed to write session file: ", fileError);
      }
