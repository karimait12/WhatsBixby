...
async function WriteSession() {
  try {
    ...
    if (process.env.SESSION !== undefined) {
      ...
      try {
        fs.writeFileSync("./lib/temp/session/creds.json", JSON.stringify(data, null, 2));
      ...
      const { data: body } = await axios.get(`${BASE_URL}session?id=${EncryptedText}`);
      const raw = JSON.parse(body).result;        // still a stringified JSON
      const sessionData = JSON.parse(raw);        // parsed into an object

      try {
        fs.writeFileSync("./lib/temp/session/creds.json", JSON.stringify(sessionData, null, 2));
        console.log("Session extraction complete");
      } catch (fileError) {
        console.error("Failed to write session file: ", fileError);
      }
