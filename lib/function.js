...
const { SESSION_ID, BASE_URL, MUSIX_URL, MUSIX_KEY } = require("../config");
...
async function WriteSession() {
  try {
    ...
    if (process.env.SESSION !== undefined) {
      ...
      // Write the session data as a single-line JSON
      fs.writeFileSync("./lib/temp/session/creds.json", JSON.stringify(data));
      console.log("Session extraction complete");
    } else {
      const EncryptedText = SESSION_ID.replaceAll("bixby~", "");
      const url = `${BASE_URL}session?id=${encodeURIComponent(EncryptedText)}`;
      
      const { data: body } = await axios.get(url);
      const raw = JSON.parse(body).result;        // `body` is a single-line JSON string
      const sessionData = JSON.parse(raw);        // Convert stringified JSON to object

      // Write the session data as a single-line JSON
      fs.writeFileSync("./lib/temp/session/creds.json", JSON.stringify(sessionData));
      console.log("Session extraction complete");
    }
  } catch (error) {
...
async function getLyrics(song, artist) {
  try {
    const searchUrl = `${MUSIX_URL}track.search?q_track=${encodeURIComponent(
      song
    )}&q_artist=${encodeURIComponent(artist)}&f_has_lyrics=1&apikey=${MUSIX_KEY}`;
    ...
    if (trackList.length > 0) {
      ...
      const allTracksUrl = `${MUSIX_URL}track.search?q_artist=${encodeURIComponent(
        artist
      )}&apikey=${MUSIX_KEY}`;
    ...
    if (trackId) {
      const lyricsUrl = `${MUSIX_URL}track.lyrics.get?track_id=${trackId}&apikey=${MUSIX_KEY}`;
...
module.exports = {
  ...
  AddMp3Meta: async (
    songbuffer,
    coverBuffer,
    options = { title: "WhatsBixby Whatsapp bot", artist: ["Codex"] }
  ) => {
  ...
  UpdateLocal: async () => {
    async function checkAndInitializeGitRepo() {
      ...
      if (!isGitRepo) {
          try {
              await git.init();
              await git.addRemote('origin', 'https://github.com/c-o-d-e-xx/WhatsBixby.git');
          } catch (error) {
  ...
  try {
      await checkAndInitializeGitRepo();
        await git.fetch();
      const commits = await git.main(['..origin/master']);
      if (commits.total === 0) {
    ...
    fromMe: async function fromMe(jid) {
      try {
          let creds = require("../lib/temp/session/creds.json");
